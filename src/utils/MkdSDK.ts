...
export default class MkdSDK {
  ...
  private _fe_baseurl: string;
  private _project_id: string;
  private _secret: string;
  private _table: string;
  private _GOOGLE_CAPTCHA_SITEKEY: string;
  private _base64Encode: string;

  constructor(config: MkdSDKConfig = {}) {
    this._baseurl = config.baseurl || "https://baas.mytechpassport.com";
    this._fe_baseurl = config.fe_baseurl || "http://localhost:3000";
    this._project_id = config.project_id || "core";
    this._secret = config.secret || "3c3w0u0scvmcv39q0brods1iv11q6n5lf";
    this._table = config.table || "";
    this._GOOGLE_CAPTCHA_SITEKEY =
      config.GOOGLE_CAPTCHA_SITEKEY ||
      "6LfmBc8jAAAAAKfz4zIiX1HoAwuH-9kcx68-7hhd";
  ...
  /**
   * Update X-Project credentials
   * @param projectId New project ID
   * @param secret New project secret
   */
  updateXProject(projectId: string, secret: string): void {
    this._base64Encode = btoa(`${projectId}:${secret}`);
  }

  /**
   * Reset X-Project credentials to original
   */
  resetXProject(): void {
    const raw = `${this._project_id}:${this._secret}`;
    this._base64Encode = btoa(raw);
  ...
  async editorUploadImage(file: File): Promise<MkdAPIResponse> {
    ...
    return this.request({
      endpoint: `/v1/api/{{project}}/{{role}}/lambda/s3/upload`,
   ...
   * @param type Social login type (e.g., google, facebook)
